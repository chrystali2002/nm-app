...
if st.button("🚀 Run QA/QC Analysis"):
    ...
    for idx, row in neighbor_df.iterrows():
        progress = 0.3 + (idx / total_stations * 0.6)
        progress_bar.progress(progress)
        ...
        status_text.text(f"Processing {station_name}...")
        ...
        # Enhanced plotting for each station
        ...
        stats_text = (
            f"Total points: {flag_stats['total_points']:,}\n"
            f"Flagged: {flag_stats['final_flag_count']:,} ({flag_stats['final_flag_pct']:.1f}%)\n"
            f"Range flags: {flag_stats['flag_range_count']:,}\n"
            f"Spike flags: {flag_stats['flag_spike_count']:,}\n"
            f"Flatline flags: {flag_stats['flag_flat_count']:,}\n"
            f"Spatial flags: {flag_stats['flag_spatial_count']:,}\n"
            f"Heatwaves: {flag_stats['heatwave_count']:,}\n"
            f"Decision: {flag_stats['decision']}"
        )
        ...
        ax1.set_title(f"{first_year_with_data} - {station_name}\n"
                     f"Neighbor: {row['NEIGHBOR NAME'] if pd.notna(row['NEIGHBOR NAME']) else 'None'} "
        ...
        flag_counts = [
            flag_stats['flag_range_count'],
            flag_stats['flag_spike_count'],
            flag_stats['flag_flat_count'],
            flag_stats['flag_spatial_count']
        ]
        ...
        if sum(flag_counts) > 0:
            ...
            if len(non_zero_idx) > 0:
                ...
                ax3.set_title(f'Flag Distribution\nDominant: {flag_stats["dominant_flag"]}')
            else:
        ...
        if len(df_primary) > 0:
            ...
            ax5.set_xlabel('Date')
            ax5.set_title(f'Flag Clustering')
...
if 'all_station_stats' in st.session_state:
    ...
    with tab1:
        ...
        display_cols = ['station_name', 'total_points', 'final_flag_pct', 
    ...
    with tab2:
        ...
        if selected_station:
            ...
            station_stats = st.session_state.all_station_stats[
                st.session_state.all_station_stats['station_name'] == selected_station
            ].iloc[0]
            ...
            # Re-create the enhanced plot (already shown during processing)
            # But we can add a note about the decision matrix
    ...
    with tab5:
        ...
        decision_filter = st.selectbox(
            "Filter by decision",
            options=['All'] + list(st.session_state.all_station_stats['decision'].unique())
        )
        ...
        st.dataframe(
            filtered_stats[['station_name', 'final_flag_pct', 'dominant_flag', 
                           'dominant_flag_pct', 'secondary_flag', 'decision']]
        )
        ...
        for _, row in st.session_state.neighbor_df.iterrows():
            ...
            if not station_stats.empty:
                ...
                ax.scatter(row['PRIMARY LON'], row['PRIMARY LAT'], 
                          color=color, s=marker_size, edgecolor='black', linewidth=0.5, 
                          zorder=5, alpha=0.7)
