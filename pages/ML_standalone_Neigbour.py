...
def find_nearest_station(primary_lat, primary_lon, stations_df, exclude_filename):
    """Find the nearest station to the primary station."""
    min_dist = float('inf')
    nearest = None
...
# Option to manually select neighbor or auto-find
...
if neighbor_option == "Auto-find nearest":
    ...
    # Create list of potential neighbors (excluding primary)
...
# Load neighbor station data (if available)
df_neighbor = None
neighbor_name = None
neighbor_dist = None
...
if neighbor_station is not None:
    ...
    if all_neighbor_data:
        ...
        st.warning(f"⚠️ No data found for neighbor station - spatial checks will be disabled")
...
# [Rest of the tabs remain exactly the same as in the previous version]
# ... (I'll include them but they're identical to before)
