...
# ---------------------------------------------------------
st.set_page_config(
    page_title="Atmospheric Gas Comparison (Sentinel-5P) for USA / U.S. States",
    page_icon="🌎",
    layout="wide"
)

st.image("https://github.com/chrystali2002.png", width=100)
st.header("Timeframe Comparison of Atmospheric Gases over USA or Any U.S. State", divider="rainbow")
...
REGION_OPTIONS = ["All USA"] + STATE_NAMES

REGION_CENTER_ZOOM = {
    "All USA": ([39.5, -98.35], 4),
    "Alabama": ([32.8, -86.8], 6),
    "Alaska": ([64.2, -152.0], 3),
...
def get_region_geometry(region_name):
    if region_name == "All USA":
        ...
        feature = states_fc.filter(ee.Filter.eq("NAME", region_name)).first()
...
map_center, zoom_level = REGION_CENTER_ZOOM.get(region_name, ([39.5, -98.35], 4))
...
def compute_tiles_and_meta(
    ...
    # Exact agreement between tiles and legends
